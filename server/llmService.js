/**
 * LLM Service - Handles secure interactions with LLM APIs (Gemini / OpenAI)
 * and guarantees structured JSON generation with Difficulty and Topic-level tagging.
 */
...
/**
 * Returns prompt instructions tailored to the chosen difficulty
 */
...
/**
 * Generate study material using Google Gemini API
 */
...
Note: 'correctAnswer' is the zero-based integer index of the correct option. 'topic' is required.`;
 ...
 * Generates topic-categorized study sets adjusted for difficulty and weak topics
 */
function generateDynamicMock(content, mode, difficulty = 'medium', weakTopics = null) {
  ...
  // Quiz mode mock with structured topic categories
...
export async function generateStudyMaterial(content, mode, difficulty = 'medium', weakTopics = null) {
  ...
  if (provider === 'gemini' && geminiKey && geminiKey !== 'YOUR_GEMINI_API_KEY_HERE') {
    rawJson = await generateWithGemini(content, mode, difficulty, weakTopics, geminiKey);
  }
  // 2. Check OpenAI
  else if (provider === 'openai' && openAIKey && openAIKey !== 'YOUR_OPENAI_API_KEY_HERE') {
    rawJson = await generateWithOpenAI(content, mode, difficulty, weakTopics, openAIKey);
  }
  // 3. Fallback to Gemini if key exists
  else if (geminiKey && geminiKey !== 'YOUR_GEMINI_API_KEY_HERE') {
    rawJson = await generateWithGemini(content, mode, difficulty, weakTopics, geminiKey);
  }
  // 4. Fallback to OpenAI if key exists
  else if (openAIKey && openAIKey !== 'YOUR_OPENAI_API_KEY_HERE') {
    rawJson = await generateWithOpenAI(content, mode, difficulty, weakTopics, openAIKey);
  }
  // 5. Intelligent Mock Engine for seamless zero-config evaluation
  else {
    console.log(`[LLMService] No API key - using dynamic mock engine (Difficulty: ${difficulty}, Mode: ${mode}).`);
    await new Promise((resolve) => setTimeout(resolve, 800));
    rawJson = generateDynamicMock(content, mode, difficulty, weakTopics);
  }
